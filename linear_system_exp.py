...
if args.exp == 'RandBiMod':
    ...
    # args.T = 1_000  # horizon
    args.T = 1_000_000  # horizon
    # args.T = 10_000_000  # horizon
...
args.run_name = 'LinearSystem'  # 'Name of dir to save results in (if empty, name by time)'
...
def run_simulations(arguments, save_result):
    ...
    for i_rep in range(arguments.n_reps):
        ...
        # if args.optimizer != 'SGD':     # TODO: support interpolation for optimizers other than SGD
        #     interpolated_error = interpolate.interp1d(x_axis, loss, kind='cubic')
        #     interpolated_suboptimality = interpolate.interp1d(x_axis, suboptimality, kind='cubic')
        #     new_axis = np.array(np.arange(args.n_samples+1))
        #     loss = interpolated_loss(new_axis)
        #     suboptimality = interpolated_suboptimality(new_axis)
...
def main(args, save_result=True, load_run_data_flag=False, result_dir_to_load='',
    ...
    # ----- Plot figures ----- #
    if plot or save_PDF:
        plot_convergence(error_avg, error_std, n_reps=args.n_reps, subsample=args.subsample, label='SGD')
