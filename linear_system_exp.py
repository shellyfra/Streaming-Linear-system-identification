...
args.optimizer = 'SGD_ER'
if args.optimizer == 'SGD':
    args.result_dir = 'SGD_and_OLS'
...
def run_simulations(arguments, save_result):
    ...
    #              }
        ...
        info_dict = {str(args.optimizer) + '_error_avg': error_mat.mean(axis=0),
                     str(args.optimizer) + '_error_std': error_mat.std(axis=0)}
    if save_result:
...
def main(args, save_result=True, load_run_data_flag=False, result_dir_to_load='',
    ...
    if plot or save_PDF:
        ...
        plot_convergence(SGD_ER_errorr_avg, SGD_ER_errorr_std, n_reps=args.n_reps, subsample=args.subsample, label='SGD_ER')
